...
describe('MatchPage', () => {
  ...
  it('displays content matches', async () => {
    ...
    expect(screen.getByText('Watch together (focused)')).toBeInTheDocument();
    expect(screen.getByText('Watch together (background)')).toBeInTheDocument();
    expect(screen.getAllByText('Watching').length).toBeGreaterThanOrEqual(2); // At least two "Watching" texts
  });
  ...
  it('allows sorting content matches', async () => {
    ...
    // Verify movie and TV show titles are present before sorting
    expect(screen.getByText('Test Movie')).toBeInTheDocument();
    expect(screen.getByText('Test TV Show')).toBeInTheDocument();
