import { fetchWithAuth, type PaginatedResponse } from './utils';
...
export const admin = {
  ...
  exportUsersAsCsv: async (
    ...
    const response = await fetch(
      `${(import.meta as unknown as { env: Record<string, string> }).env.VITE_API_URL || 'http://localhost:3000'}/api/admin/users-csv?${queryParams.toString()}`,
      { headers }
    );
