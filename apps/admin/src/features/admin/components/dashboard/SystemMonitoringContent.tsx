...
import * as styles from './SystemMonitoringContent.css';
...
const SystemMonitoringContent: React.FC = () => {
  ...
  const generateSystemAlerts = (data: SystemStats): SystemAlert[] => {
    ...
    // CPU usage alert
    const cpuUsage =
      ((data.system?.os?.loadAvg?.[0] || 0) * 100) / data.system?.cpu?.cores ||
      0;
    if (cpuUsage > 90) {
      alerts.push({
        severity: 'critical',
  ...
  return (
    <>
      ...
      <Typography className={styles.lastUpdated} variant="body2">
            ...
            <Card className={styles.statsCard}>
              <CardContent>
                ...
                <div className={styles.metric}>
                  ...
                  <Typography>
                    {(monitoringData.cpu.loadAvg[0] || 0).toFixed(2)}
                  </Typography>
                </div>
                <div className={styles.metric}>
                  <Typography className={styles.metricName} variant="body2">
