import Content, { type ContentProviders } from '../models/Content';
...
export const refreshProvidersForContent = async (
	...
	const regions = await fetchWatchProviders(tmdbId, mediaType);
...
export const getProvidersForContent = async (
	...
	if (!cached || isStale(cached, maxAgeHours) || !cached.regions[region]) {
		...
		return next?.regions[region] ?? null;
	}

	return cached.regions[region] ?? null;
};
