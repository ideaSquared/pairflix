...
export const mobileFirst = (styles: Partial<Record<BreakpointKey, string>>): string => {
  const breakpointOrder: BreakpointKey[] = ['xs', 'sm', 'md', 'lg', 'xl', 'xxl'];
  
  return breakpointOrder
    .filter(bp => styles[bp])
    .map(bp => `@media ${media[`${bp}Up` as keyof typeof media]} { ${styles[bp]} }`)
    .join('\n');
};

export const desktopFirst = (styles: Partial<Record<BreakpointKey, string>>): string => {
