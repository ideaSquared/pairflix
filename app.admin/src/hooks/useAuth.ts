...
export function useAuth() {
	...
	const {
		...
		queryKey: ['auth'],
		queryFn: async () => {			const token = localStorage.getItem(ADMIN_TOKEN_KEY);
			if (!token) {
			...
			try {
				...
				// Handle validation errors				console.error("Auth validation error:", error);
				localStorage.removeItem(ADMIN_TOKEN_KEY);
				localStorage.removeItem('user');
				throw new Error('Authentication required');
			}
	...
	const isAuthenticated = Boolean(user && localStorage.getItem(ADMIN_TOKEN_KEY));
