...
export const createResponsiveCSS = <T>(
  ...
  if (typeof value === 'object' && value !== null) {
    ...
    sortedKeys.forEach(breakpoint => {
      // typeof-narrowed to the responsive-map branch of ResponsiveValue<T>; T may
      // itself be an object type so the compiler can't discriminate the union further
      const val = (value as Partial<Record<BreakpointKey, T>>)[breakpoint];
      if (val !== undefined) {
...
export const getSpacing = (
  value: SpacingValue,
  theme: { spacing?: Partial<Record<SpacingValue, string>> }
): string => {
...
export const responsiveSpacing =
  ...
  (theme: { spacing?: Partial<Record<SpacingValue, string>> }): string => {
