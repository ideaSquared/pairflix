...
import { centeredCard } from './Loading.stories.css';
...
const CenteredCard = ({ children }: { children: ReactNode }) => (
  <Card className={centeredCard}>{children}</Card>
);

const DemoSection = ({ children }: { children: ReactNode }) => (
  <div style={{ marginBottom: 24 }}>{children}</div>
);

const SectionTitle = ({ children }: { children: ReactNode }) => (
  <h3 style={{ marginBottom: 16, fontSize: 18 }}>{children}</h3>
);
...
export const LoadingInContainer: Story = {
  render: () => (
    <Container>
      ...
      <FullHeightContainer>
        <Loading size={48} message="Loading content">
          <div
            style={{
              marginTop: '16px',
              fontSize: '14px',
