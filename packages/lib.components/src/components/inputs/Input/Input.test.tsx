...
// Helper to render components with the real theme class applied
const renderWithTheme = (component: React.ReactNode) => {
  return render(
    <div className={`${lightThemeClass} ${themeRoot}`}>{component}</div>
  );
};
...
describe('Input Component', () => {
  ...
  it('applies full width styling when isFullWidth is true', () => {
    ...
    // The wrapping InputGroup should also be full width
    const inputGroup = input.parentElement?.parentElement;
    expect(inputGroup).toBeInTheDocument();
    // Non-null asserted by the toBeInTheDocument() check above.
    expect(getComputedStyle(inputGroup as HTMLElement).width).toBe('100%');
  });
