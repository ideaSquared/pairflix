...
// Route params are always a single path segment in this app's routes; narrow
// away the `string[]` that Express's types allow for repeating-segment patterns.
const paramAsString = (
...
export const getContentReports = async (req: Request, res: Response) => {
	try {
		const contentId = paramAsString(req.params.contentId);
...
export const dismissReport = async (req: Request, res: Response) => {
	try {
		const reportId = paramAsString(req.params.reportId);
